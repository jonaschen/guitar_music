...
const score = {
  ...
  chords: [{ id: "c1", start: 0, end: 2, symbol: "C", confidence: 1, origin: "model", edited: false, voicing_id: null, available_voicings: [], roman_numeral: "I", harmonic_function: "tonic", theory_confidence: 0.85 }],
...
test("renders an analyzed score workspace", async ({ page }) => {
  ...
  await expect(page.getByText("I · tonic", { exact: true }).first()).toBeVisible();
  ...
  await page.getByRole("button", { name: "Split line" }).click();
  await expect(page.getByRole("button", { name: "Edited", exact: true })).toBeVisible();
  await expect(page.getByRole("button", { name: "lyric", exact: true })).toBeVisible();
  await page.getByRole("button", { name: "Undo" }).click();
  await expect(page.getByRole("button", { name: "Edited lyric", exact: true })).toBeVisible();
  await page.getByRole("button", { name: "Undo" }).click();
  await expect(page.getByRole("button", { name: "One two", exact: true })).toBeVisible();
