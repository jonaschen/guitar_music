...
### 2026-09-25 按拍長呈現小節和弦（待人工驗收）

- 使用者確認：兩個等寬和弦卡片讓人預期 2:2，但實際可能是 3:1，至少部分落差來自譜面表達，不是播放漂移。
- 小節內依現有拍點做分段插值，和弦寬度按拍長排列，加入拍號刻度、目前拍高亮與各段拍長。變速拍點不會被當成額外拍數；無拍點退回明示的秒數比例。
- 保留跨小節延續、空白／N.C. 區段、原始區段編輯與 Review 標記；不改和弦時間、不強制 2:2／2:1:1，也不新增或重排刷奏事件。
- 增加 3:1、2:2、2:1:1、不等拍秒數、弱起、尾端不完整小節與無拍點測試；Bar 12 在桌面與手機驗證 G:C 寬度約 3:1。實際保存歌曲截圖已確認同樣呈現。
- 下一人工 checkpoint：Bar 11–14 是否能依刻度預期第 4 拍換和弦。剩餘頻繁末拍變化與段落結構屬分析品質問題，尚不能認定皆合理或皆錯誤。

### 2026-09-25 Bar 12 音畫落差追查

- 使用者回報整曲約 Bar 12 起，聽到的和弦與畫面行進不一致，仍未通過驗收。
- 保存 job `38d2ef940b6f471e9d9fb9f501700b77` 的 Bar 12 為 24.5899–26.8423 秒：G 在前 3 拍，C 於 26.2734 秒（第 4 拍）進入並延續到下一小節。用控制組 4/4 刷奏編譯時，該處 G→C 的事件起點與譜面一致；不代表瀏覽器中經修改的樂譜或實際聽感已重現。
- 找到獨立可重現的狀態漏洞：改譜／改刷奏會更新 UI，但已編譯或準備中的舊播放未取消。現在 score snapshot 變更一律停止並失效舊播放，重新按 Play score 才編譯新內容。
- 新增「譜面當前／演奏事件」對照，後者來自播放 manifest 與其 snapshot，而非編輯器選取和弦。此讀值是排程診斷，不是麥克風量測；收音尾巴可能短暫重疊。
- 新增 Bar 12 時間案例與播放中／準備中改譜取消的瀏覽器回歸。仍需同一段人工回報，不能把這個防護修正當作已找到全部音畫異常原因。

### 2026-09-25 整曲檢查：重複區段與未確認的小節起點

- 人工回饋：四小節控制組的一／二／三和弦均明顯改善連續性與自然度；整曲仍缺乏小節感，不能外推為整曲驗收通過。
- 保存 job `38d2ef940b6f471e9d9fb9f501700b77` 的 117 個推定小節中，60 個有至少三個原始區段；僅按相鄰同標籤連續性合併後，剩 10 個至少三段和聲。這是資料統計，不是和弦正確性評分。
- UI 預設合併相鄰同和弦／指型，保留跨度、各段 ID、Review／Edited 狀態與原始區段顯示開關。不改分析資料、不跨休止或不同指型合併、不把所有三和弦硬排成 2＋1＋1。
- 整曲新增明確套用已試聽 4/4 刷奏的按鈕，仍屬使用者可撤銷的節奏選擇。保存歌曲原為 `pop_4_4_quarter_01`，不是控制組刷奏。
- 該 job 的分析警告指出沒有 downbeat detection，第一個 pulse 被假定為第 1 拍；在譜面明示未確認。改刷法或合併顯示不會校正這個問題。
- 下一個品質重點：整曲使用相同刷奏後，對照原曲確認第一拍與小節相位，再檢查剩餘多和弦區段的換和弦時點；不得宣稱已自動校正。

### 2026-09-25 試聽修正：強弱刷奏與弦音共鳴分離

- 使用者回報：4/4、每小節一個和弦，強弱交替仍有明顯斷開感。
- 找到播放層原因：撥弦音色仍在下一刷時啟動上一刷的收音，弱刷到來會同時失去強刷的餘音。
- Playback manifest 新增選用 `sustain_end`，同和弦／指型的連續區段共用共鳴終點；換和弦的下一有效刷奏或 N.C.／間隙開始收音。保留原 `end` 與 MIDI 拍長，不改強弱比例。
- 四小節對照與整首 Play score 的新撥弦音色同步採用；跳轉會恢復仍在衰減的弦音。舊音色保持原行為供比較。
- 待人工試聽驗收：先測每小節一和弦，再測兩和弦。這是播放連續性修正，不代表旋律辨識或和弦拍點品質已通過。
...
小節結構 follow-up：使用者指出 1／3 拍聽感仍太相近，並要求三和弦時考慮時值與刷法。控制組將 balanced 拍位力度比例明確拉開為 1／0.28／0.55／0.28，附四拍和弦與強弱格，Bar 明示為小節。新增三和弦 2＋1＋1 拍配置，後兩拍使用較簡潔下刷／延音；第 4 拍換新和弦仍為弱拍，不重設成強拍。新增兩種手動可選整曲模板，未取代原 balanced 模板或覆寫舊 score；auto_suggest=false 排除其自動選型，以免只按和弦數推定 2＋1＋1。半小節／三和弦規劃仍是控制組範例，非一般歌曲的強制規則，待人工試聽。

第二和弦動態 follow-up：使用者反映先輕後重；確認原控制型第 3 拍空刷、3 ＆ 輕上刷、4 拍较強的模板配置是直接原因，不是 envelope 反向。新增 Balanced 4/4（第 1 拍最強、第 3 拍次強、其餘較輕）作控制組預設與可選整曲 rhythm；保留原型作對照。新增半小節第二和弦必須在第 3 拍下刷、後續力度較低的測試。未以 chord boundary 全域重設重音，仍依拍位編排；等待聽感確認。
