...
- [ ] **拍點與小節修正**（主文件 §4.3）
  ...
  - 已可將播放頭所在 beat 設為目前時間，或以 range handle 逐拍拖曳；兩者皆保護相鄰 beat 的排序。跨多 beat／小節選取拖曳仍待完成。
- [x] **Undo / Redo**（追加文件 §11）
...
- [x] **歌詞編輯與手動對時 UI**
  - 已提供匯入、逐行文字編輯、Set start/end、播放高亮、分配 timing 與可開關的最近拍點 snapping；完成對時的歌詞行可直接拖曳 start/end range handles。所有修改皆標示為 user edit 並納入 Undo/Redo。
